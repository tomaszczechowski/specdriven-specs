...
const METADATA_REQUIRED_EXTERNAL = [
  "title",
  "description",
  "category",
  "stack",
...
function validateSpecFile(slug, entryPath) {
  ...
  if (fm.name !== slug) {
    error(
      `  ❌ ${ENTRY_FILENAME}: name "${fm.name}" doesn't match directory "${slug}"`,
    );
    valid = false;
  }
  ...
  if (
    ...
    fm.description.length > 150
  ) {
    error(
      `  ❌ ${ENTRY_FILENAME}: description must be a non-empty string, max 150 chars`,
    );
    valid = false;
  }
  ...
  if (wordCount < MIN_WORDS) {
    error(
      `  ❌ ${ENTRY_FILENAME}: content too short: ${wordCount} words (minimum ${MIN_WORDS})`,
    );
    valid = false;
  }
...
function validateMetadataFile(slug, metaPath, isExternal) {
  ...
  let valid = true;
  const required = isExternal
    ? METADATA_REQUIRED_EXTERNAL
    : METADATA_REQUIRED_LOCAL;
  ...
  if (metadata.source !== undefined) {
    ...
    if (
      ...
      error(
        `  ❌ ${METADATA_FILENAME}: source.url must be a valid http(s) URL`,
      );
      valid = false;
    }
    if (
      s &&
      s.homepage !== undefined &&
      (typeof s.homepage !== "string" || !/^https?:\/\//.test(s.homepage))
    ) {
      error(
        `  ❌ ${METADATA_FILENAME}: source.homepage must be a valid http(s) URL`,
      );
      valid = false;
    }
  ...
  if (
    ...
    error(
      `  ❌ ${METADATA_FILENAME}: description must be a non-empty string (≤150 chars) for external entries`,
    );
    valid = false;
  }
  ...
  if (metadata.title && metadata.title.length > 60) {
    warn(
      `  ⚠️  ${METADATA_FILENAME}: title is long (${metadata.title.length} chars)`,
    );
  }
...
function validateSpecDirectory(specDir) {
  ...
  const isExternal = !!(
    probeMetadata &&
    probeMetadata.source &&
    typeof probeMetadata.source.url === "string"
  );

  if (!isExternal && !fs.existsSync(entryPath)) {
    error(
      `  ❌ Missing entry file: ${ENTRY_FILENAME} (required for local entries; set metadata.source.url for external)`,
    );
  ...
  for (const e of entries) {
    ...
    if (size > MAX_FILE_BYTES) {
      error(
        `  ❌ File too large: ${rel} (${size} bytes, max ${MAX_FILE_BYTES})`,
      );
      valid = false;
    }
  }
  ...
  if (totalBytes > MAX_TOTAL_BYTES) {
    error(
      `  ❌ Total size too large: ${totalBytes} bytes (max ${MAX_TOTAL_BYTES})`,
    );
    valid = false;
  }

  // Local entries require SPEC.md; external entries optionally include it as curated commentary.
  if (fs.existsSync(entryPath) && !validateSpecFile(slug, entryPath))
    valid = false;
  ...
  if (valid) {
    ...
    success(
      `  ✅ Valid${tag} (${fileCount} file${fileCount === 1 ? "" : "s"}, ${totalBytes} bytes)`,
    );
  }
